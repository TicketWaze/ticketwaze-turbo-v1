 ...
 * given by `useSession()` and send it to the API, and by default
 * `SessionProvider` refetches only on mount and on window focus — so a screen
 * someone stayed on and worked in, a checkout being filled in or a long form,
 * was still presenting the token it was handed when the page loaded. The API
 * answered "Unauthorized access", which surfaced as an unexplained failure and
 * an auth alert in #logs.
 *
