...
export default function RichTextEditor({
  ...
  const editor = useEditor({
    /**
     * Defers the first render to the client.
     *
     * The organiser app's copy of this component guards the same hydration
     * mismatch with a `mounted` flag set from an effect; this app's lint config
     * rejects setState-in-effect, and Tiptap v3 has the purpose-built option
     * anyway. `editor` is null until it has rendered, which is what the
     * skeleton below keys off — so the flag was never carrying anything the
     * editor's own lifecycle does not already say.
     */
  ...
  // Null on the first pass, before the editor exists — see immediatelyRender.
  const charCount = activeState?.charCount ?? 0;
