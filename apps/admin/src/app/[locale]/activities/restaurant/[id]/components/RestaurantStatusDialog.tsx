...
import { UpdateRestaurantStatusAction } from "@/actions/Activity";
import { Restaurant } from "@ticketwaze/typescript-config";
import { AnimatePresence, motion } from "framer-motion";
import { ArrowLeft2 } from "iconsax-reactjs";
import { cn } from "@/lib/utils";

type AdminStatus = Restaurant["adminStatus"];
...
> = {
  review: {
    label: "Under Review",
    description: "Venue is pending review by the admin team",
    color: "#EA961C",
    bg: "#FEF3E2",
  },
  approved: {
    label: "Approved",
    description: "Venue is approved and publicly visible",
    color: "#349C2E",
    bg: "#E8F5E2",
  },
  rejected: {
    label: "Rejected",
    description: "Venue has been rejected and is not visible",
    color: "#E53935",
    bg: "#FCE5EA",
  },
  requested: {
...
export function RestaurantStatusDialog({
  restaurant,
  className,
}: {
  ...
  const [selected, setSelected] = useState<AdminStatus>(restaurant.adminStatus);
  ...
  function handleOpenChange(next: boolean) {
    setOpen(next);
    if (!next) {
      setSelected(restaurant.adminStatus);
      setRejectionReason("");
      setStep("select");
    }
  }
  ...
  async function handleConfirm() {
    if (selected === restaurant.adminStatus && step === "select") {
      setOpen(false);
      return;
    }
    setIsLoading(true);
    const result = await UpdateRestaurantStatusAction(
  ...
  const currentConfig = STATUS_CONFIG[restaurant.adminStatus];

  // Unlike events and raffles there is no frozen state: a restaurant never
  // ends, so its review outcome can always be revisited.
          ...
          {step === "select" ? (
            ...
            >
              <div className="flex flex-col gap-2">
                <DialogTitle>Change Venue Status</DialogTitle>
              ...
              <DialogFooter>
                ...
                <ButtonPrimary
                  className="flex-1"
                  disabled={isLoading || selected === restaurant.adminStatus}
            ...
            >
              <div className="flex flex-col gap-2">
                ...
                <p className="text-[1.3rem] leading-6 text-neutral-500">
                  Explain why this venue is being rejected. This will be visible
                  to the organizer.
                </p>
              </div>
