...
import { UpdateRaffleStatusAction } from "@/actions/Activity";
import { Raffle } from "@ticketwaze/typescript-config";
import { AnimatePresence, motion } from "framer-motion";
import { ArrowLeft2 } from "iconsax-reactjs";
import { cn } from "@/lib/utils";

type AdminStatus = Raffle["adminStatus"];
...
> = {
  review: {
    label: "Under Review",
    description: "Raffle is pending review by the admin team",
    color: "#EA961C",
    bg: "#FEF3E2",
  },
  approved: {
    label: "Approved",
    description: "Raffle is approved and publicly visible",
    color: "#349C2E",
    bg: "#E8F5E2",
  },
  rejected: {
    label: "Rejected",
    description: "Raffle has been rejected and is not visible",
    color: "#E53935",
    bg: "#FCE5EA",
  },
  requested: {
...
export function RaffleStatusDialog({
  ...
  function handleOpenChange(next: boolean) {
    setOpen(next);
    if (!next) {
      setSelected(raffle.adminStatus);
      setRejectionReason("");
      setStep("select");
    }
  }
  ...
  async function handleConfirm() {
    if (selected === raffle.adminStatus && step === "select") {
      setOpen(false);
      return;
    }
    setIsLoading(true);
  ...
  const currentConfig = STATUS_CONFIG[raffle.adminStatus];

  // Once drawn/closed/cancelled the review outcome is frozen (API rejects too).
  const isFrozen = raffle.status !== "on_sale";

  if (isFrozen) {
    return (
      <ButtonNeutral
        disabled
        title="This raffle can no longer be reviewed."
        className={cn(
          ...
          {step === "select" ? (
            ...
            >
              <div className="flex flex-col gap-2">
                <DialogTitle>Change Raffle Status</DialogTitle>
              ...
              <DialogFooter>
                ...
                <ButtonPrimary
                  className="flex-1"
                  disabled={isLoading || selected === raffle.adminStatus}
            ...
            >
              <div className="flex flex-col gap-2">
                ...
                <p className="text-[1.3rem] leading-6 text-neutral-500">
                  Explain why this raffle is being rejected. This will be visible
                  to the organizer.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <textarea
                  ...
                  placeholder="Explain why this raffle is being rejected..."
                  rows={4}
                  autoFocus
