...
/**
 * Live raffle-title availability check, the raffle twin of
 * useEventNameAvailability. Fires on every keystroke and aborts the previous
 * in-flight request, so only the response for the latest value ever lands.
 *
 * Separate from the event hook rather than generalised: they hit different
 * endpoints with different query parameters, and folding them together would
 * mean a caller passing a "kind" flag that decides more than it explains.
 */
export default function useRaffleTitleAvailability(
  title: string,
  /**
   * The raffle being edited. Excluded from the lookup so it does not match its
   * own title — without it the edit screen reports "taken" for a title the
   * organiser never touched.
   */
  ...
  useEffect(() => {
    ...
    if (excludeRaffleId) query.set("raffleId", excludeRaffleId);

    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/raffles/validation/title?${query.toString()}`,
      {
  ...
  }, [value, accessToken, excludeRaffleId]);
