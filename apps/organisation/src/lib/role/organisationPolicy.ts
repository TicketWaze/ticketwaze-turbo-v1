...
export class OrganisationPolicy {
  ...
  async updateOrganisationInformations(userId: string, organisationId: string): Promise<boolean> {
    const role = await getUserRole(userId, organisationId)
    return can(role, UserActions['organisation:update'])
  }

  async updateEventInformations(userId: string, organisationId: string): Promise<boolean> {
    const role = await getUserRole(userId, organisationId)
    return can(role, UserActions['event:update'])
  }

  async viewAnalytics(userId: string, organisationId: string): Promise<boolean> {
    const role = await getUserRole(userId, organisationId)
