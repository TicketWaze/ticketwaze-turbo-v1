...
import { CreateRestaurant } from "@/actions/EventActions";
...
// Monday first: a venue owner reads their week that way, even though the
...
function makeRestaurantSchema(t: TranslateFn) {
  return z
    ...
    // Reservations are paid: without a fee there is nothing to confirm the
    // booking with, so the toggle and the fee have to travel together.
...
export default function CreateRestaurantForm() {
  ...
  const {
    ...
    defaultValues: {
      ...
      deliveryEstimatedMinutes: undefined,
    },
  });
  ...
  // ── Opening hours (one range per day, WhatsApp-style) ─────────────────
  ...
  async function handleCover(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    // Downscale on pick, not on submit, so the cost is paid while the organiser
    // is still filling the form rather than as a stall after they hit save.
  ...
  async function handleGallery(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Let the same file be re-picked after a removal. Read before awaiting —
    // React pools nothing here, but the input is cleared by the time the
    // compression resolves.
    e.target.value = "";
    // Cap at 5 rather than rejecting the whole selection — someone who
    // multi-selects 8 photos meant to add photos, not to fail. Trim before
    // compressing so we never pay for images we discard.
  ...
  const onSubmit: SubmitHandler<TFormOut> = async (data) => {
    ...
    if (
      galleryFiles.length < MIN_GALLERY ||
    ...
    // Absence of a day IS the closed state, so only enabled days are sent.
    ...
    // Repeated field name — the API reads these with request.files("images").
    ...
    // Open/closed and reservation slots are all evaluated in the venue's zone.
  ...
  return (
    <div className="flex flex-col gap-8 overflow-y-scroll pb-12">
      ...
      >
        ...
        {/* Gallery — 3 to 5 photos, required */}
        <div className={cardClass}>
            ...
            <>
              ...
              <div className="flex flex-col gap-4">
                {DAY_ORDER.map((day) => {
                  ...
                  // Closing at or before opening means the range runs into the
                  // next day — a bar open 18:00 to 02:00.
                  const overnight =
