  ...
  return (
    <motion.div
      className={`rounded-[3rem] overflow-hidden flex flex-col gap-8 p-12 ${
        activeTab === value ? "active bg-white" : "bg-primary-700 "
      }`}
  ...
  return (
    <h3
      className={`cursor-pointer transition-all font-semibold text-[2.2rem] leading-12 flex justify-between items-center ${
        activeTab === value ? "active text-primary-500" : " text-primary-50"
      }`}
    >
      {children}
      <span
        className={`${activeTab === value ? "text-primary-900" : "text-primary-50"}`}
      >
        {number}
      </span>
    </h3>
